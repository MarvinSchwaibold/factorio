...
export function InsightsPreviewPanel({
  isActive,
  isCollapsing,
  onApprove,
  mirrored
  ...
  const insightsContent = (
    ...
    >
      ...
      <motion.div onWheel={(e) => e.stopPropagation()} style={{ border: isRetro ? "2px solid rgba(224, 112, 32, 0.5)" : "1px solid #fef3c7", background: isRetro ? "rgba(224, 112, 32, 0.08)" : theme.cardBg, padding: "16px 20px", minWidth: isRetro ? 320 : 300, maxWidth: isRetro ? 360 : 340, position: "relative", overflow: "hidden", borderRadius: theme.borderRadius, boxShadow: theme.shadow }}>
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: isRetro ? 8 : 6 }}>
            {[{ metric: "Revenue", value: "+23%", trend: "up" },{ metric: "Orders", value: "+18%", trend: "up" },{ metric: "AOV", value: "$127", trend: "up" },{ metric: "Returns", value: "-5%", trend: "down" },{ metric: "New Customers", value: "+31%", trend: "up" }].map((item, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: isRetro ? "6px 10px" : "8px 12px", background: isRetro ? "rgba(0, 0, 0, 0.2)" : theme.cardBgHover, border: isRetro ? "1px solid rgba(224, 112, 32, 0.15)" : undefined, borderRadius: isRetro ? 0 : 6 }}>
